...
variable_range = [[1],list(range(1,20))]
...
envs = [0.1,0.5,0.25,0.0,-0.25,-0.5,-1.0]
for i in range(len(envs)) :
    references = {"Au": gold_reference}
    environment = {"mu_Au": envs[i]}
    ...
    if(i == 0):
        ...
        starting_population = 20,population_size=30,steps=100,
        structures_filename = "structures_{}.traj".format(envs[i]),
        restart = False)
    else:
        gcga = GCGA(slab,variable_types,variable_range,mutations,
        gfe,
        ...
        starting_population = 20,population_size=30,steps=100,
        structures_filename = "structures_{}.traj".format(envs[i]),
        restart = True,restart_filename="structures_{}.traj".format(envs[i-1]))
